// This is the full code for an ESP sensor (ID: esp2sensor).
...
//String startingIP = "172.20.";
//String currentIP = "172.20.";
...
// 7) DHT SENSOR VARIABLES
// Sensor we're using DHT22
#define DHTTYPE DHT22   // DHT 22  (AM2302), AM2321
// DHT Sensor
uint8_t DHTPin = D1; // data pin (requires 10k connected, see schematic on website above) - WORKS WITH D1, RESET AND EVERYTHING
...
void setup() {
  ...
  dht.begin(); //initialization function for dht22 (initialize dht22 as an object)      
...
void loop() { 
  ...
  if (initialize_connection == true){
    ...
    if(httpCode != 200){ //200: successful
      ...
      client.publish("/esp8266/esp2sensor-esp", "sensor"); //our greeting from esp 2! original ID: 2-esp
    }//else
  ...
  if(IP_discovery_needed == true){
    ...
    // MQTT_flag is made true in the IP_discovery() after writing to the EEPROM
    if(MQTT_flag == true){
      ...
      client.publish("/esp8266/esp2sensor-esp", "sensor"); //our greeting from esp 2!
      MQTT_flag = false;
    }
  }
    ...
    if(IP_discovery_needed == false){
      ...
      if(network_flag == true){
        network_flag = false; //to only call IP_discovery() once in case IP_discovery_needed = true; if the connection fails within IP_discovery, it'll be managed by the flag
         ...
         if(client.loop()){
           Serial.println("Connected to MQTT");
           String message_from_esp = ""; //empty upon inicialization
            // this is the loop method of PubSubClient. This function 
            // allows the client to process incoming messages and to 
            // maintain its connection to the server, so it must be 
            // constantly called
            ...
            message_from_esp = String(Temperature);
            message_from_esp = message_from_esp + "_"; 
            message_from_esp = message_from_esp + String(Humidity).c_str(); // +"_"+ String(Humidity).c_str();
            Serial.println(message_from_esp); //working!
            client.publish("/esp2sensor-esp/sensor", String(String(Temperature)+"_"+String(Humidity).c_str()).c_str());
            delay(5000); //in ms, so wait for 5 seconds
         } else{
